    ...
    <>
      ...
      <Container className="space-y-4">
        {products.length === 0 ? (
          ...
          products.map((p) => (
            <div key={p.id} className="card p-4 sm:p-6 space-y-3 anim-rise">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <div className="font-black text-brand-navy break-words">{p.name} <span className="text-xs text-brand-slate font-normal">— {p.team.name}</span></div>
                  <div className="text-sm text-brand-slate break-words">{p.tagline}</div>
                </div>
                {p.aiQuality !== null && (
                  <span className="chip-navy">پیش‌نمرهٔ هوش مصنوعی: {fa(p.aiQuality)}</span>
                )}
              </div>
              {p.aiNotes && <p className="text-sm text-brand-slate bg-brand-ice rounded-2xl p-3 break-words">{p.aiNotes}</p>}
